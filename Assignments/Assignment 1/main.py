...
import fitnesses
import time
import functionsForPlots as plotFunc
...
def iterateWithPlots(isUniformCrossover,fitness,isTighlyLinked):    
    ...
    while True:      
        ...
        for i in range (0,populationSize-1,2):
            ...
            decisionData=plotFunc.getDataForDecisionPlot(population[i],population[i+1],ancestors[0],ancestors[1])
            correct+=decisionData[0]
            incorrect+=decisionData[1]
            areBetter=ancestors[2]
            if areBetter :
                areBetterForGeneration=True
        schemataData=plotFunc.countMembersOfSchemata(population)
        ...
        dataForOnesPlotY.append(plotFunc.countOnesInPopulation(population)/(populationSize*length))
